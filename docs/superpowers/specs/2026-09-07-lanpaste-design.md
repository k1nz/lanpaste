...
LanPaste 是局域网、无服务器的跨设备剪贴板。本机始终有一份 Raycast 式历史；已配对设备之间默认同步（可按设备开关），也可以把某一条「同步到」指定机器。文件即使进入自动同步也**只发元数据**，对端真正粘贴时才拉字节；超过体积阈值的条目连元数据也不会自动出网。
...
一个 Tauri 进程，**三扇窗 + 托盘**：浮层、设置、配对 token 置顶窗。配对窗不是设置页里的一块区域。

```
Vue overlay / settings / pairing prompt / tray
...
| Vue pairing | 目标机 token 置顶窗；发起方的 token 输入也是独立小窗 | 只 IPC |
...
4. A 弹出输入窗（不是埋在设置表单里），用户输入 token → `POST /pair/confirm`（token + A 的 Ed25519 公钥）
...
- 所有类型在**同一个**列表
- 只按 `copied_at` 倒序；时间分组为 Today / Yesterday / 具体日期（更早的条目按日）
- 「全部类型」是过滤器，不是文件夹
...
| 条件 | 行为 |
|------|------|
| 条目内容体积 ≤20MB 自动同步 | 按整条 Pasteboard 的内容体积（含文件字节）判断。文本/URL/颜色/HTML/RTF 发全文；图片发全文；文件**只发元数据**（名、大小、哈希、下载凭证），不发文件体 |
| 条目内容体积 >20MB | 本机历史保留；**连元数据也不自动出网**，除非「同步到」。因此大文件不会自己出现在对端列表里 |
...
- 自动同步时对端离线：入队并**落盘**，进程重启后仍能在上线时补发
- 「同步到」离线：当场提示
- 证书不对：断开 + 信任失效
- 源文件已被清理：提示源设备已无此文件
- 传输中断：可续传，浮层失败态，不卡死
