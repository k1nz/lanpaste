    ...
    <template v-else-if="!virtualize">
      <section v-for="group in groups" :key="group.id" class="group">
        ...
        <HistoryRow
          ...
          :key="item.id"
          v-memo="[item.id, item.id === selectedId, item.title, item.preview.text, item.preview.html, item.fileDownloadState, item.preview.imageThumb, progress?.id === item.id ? progress.received : 0]"
          :item="item"
      ...
      <div class="virtual-window" :style="{ transform: `translateY(${offsetY}px)` }">
        <template v-for="laidRow in visibleLaid" :key="laidRow.row.key">
          ...
          <HistoryRow
            ...
            v-memo="[laidRow.row.item.id, laidRow.row.item.id === selectedId, laidRow.row.item.title, laidRow.row.item.preview.text, laidRow.row.item.preview.html, laidRow.row.item.fileDownloadState, laidRow.row.item.preview.imageThumb, progress?.id === laidRow.row.item.id ? progress.received : 0]"
